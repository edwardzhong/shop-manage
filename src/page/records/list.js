...
const List =({tag,shopTypes,shopList})=>{
    ...
    const listFilter=(list,opt={})=>{
        const ret = list.filter((l,i)=>{
            if(opt.type && l.activity.store.platformtype.id != opt.type) return false;
            if(opt.name && l.activity.store.store_name != opt.name) return false;
            if(opt.num && !RegExp(opt.num).test(l.activity.activity_num)) return false;
            if(opt.st && moment(l.activity.update_time).isBefore(opt.st)) return false; 
            if(opt.st && moment(l.activity.update_time).isAfter(opt.et)) return false; 
            return true;
        })
        return ret.map(l=>({
    ...
    const submit = ()=>{
        setMoneyList(listFilter(moneys,{st:sTime,et:eTime,type:shopTypeVal,name:shopVal,num:num}))
    }
