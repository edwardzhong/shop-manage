...
const Three = ({prevStep,nextStep}) =>{
    ...
    const data =[
        { cate:'服务费', price:info.bill.single_service_fee, num:quantity === 0 ? num: quantity, total:(quantity === 0 ? num: quantity)*info.bill.single_service_fee,}
    ]
    ...
    return <>
        ...
        <div styleName="block">
            <div>
                ...
                <Input value={num} onChange={numChange}/>单<span>(1-500单)</span>
            </div>
        </div>
        <Divider/>
        <h4>2.设置成交关键词分布</h4>
        <div styleName="block">
        <p>注意：各关键词订单总数需要为<i>{quantity === 0 ? num: quantity}</i>单</p>
